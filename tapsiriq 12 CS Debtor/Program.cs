...
class Program
{
    ...
    static void Main()
    {
        ...
        // Hell yolu cox yerde Dictionary ile verildiyinden 
        // men de uzun kod bloklari yerine dictionary ile yazmaq qerarina geldim


        //var dict = debtors.ToLookup(d => d.BirthDay.Year);
        //var maxCount = dict.Max(p => p.Count());
        //Console.Write($"{maxCount} EDED - ");
        //dict.Where(x => x.Count() == maxCount).Select(x => x.Key).Print();
        ...
        //Predicate<string> isNumberUniqie = num =>
        //{
        //    foreach (var character in num)
        //        if (character != '-' && num.Count(c => c == character) != 1)
        //            return false;
        //    return true;
        //};
